...
@WebServlet("/editCarDetails")
public class AdminEditCarDetailsServlet extends HttpServlet {
		...
		if (hs == null) {
			...
			while (it.hasNext()) {
				...
				if (carNumber.equals(cb.getCarNumber())) {
					req.setAttribute("carBean", cb);
					req.getRequestDispatcher("InfoEditCarDetails.jsp").forward(req, res);
				}
			}
